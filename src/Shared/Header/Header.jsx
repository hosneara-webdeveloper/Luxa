...
const Header = () => {
    ...
    const { data: carts = [], refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://luxsa-back-end.vercel.app/cart/${user?.email}`);
    ...
    const { data: checkUser = [] } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://luxsa-back-end.vercel.app/users/${user?.email}`);
            const data = await res.json();
            return data;
        }
    })
                ...
                <div className="navbar bg-base-100">
                    ...
                    <div className="flex-none gap-2">
                        <ul className={`menubar ${menu} md:flex md:static absolute gap-2 bg-base-100 z-[100] md:z-auto shadow-sm md:shadow-none p-3 md:bg-transparent left-0 w-full top-full`}>
                            <li className='my-2 md:my-0'><NavLink to={'/'} className="font-semibold px-2 nav-link text-black-500 hover:text-secondary hover:underline hover:underline-offset-8">home</NavLink></li>
                            <li className='my-2 md:my-0'><NavLink to={'/shop/all'} className="font-semibold px-2 nav-link text-black-500 hover:text-secondary hover:underline hover:underline-offset-8">shop</NavLink></li>
                            <li className='my-2 md:my-0'><NavLink to={'/blogs'} className="font-semibold px-2 nav-link text-black-500 hover:text-secondary hover:underline hover:underline-offset-8">blogs</NavLink></li>
                            <li className='my-2 md:my-0'><NavLink to={'/about'} className="font-semibold px-2 nav-link text-black-500 hover:text-secondary hover:underline hover:underline-offset-8">about</NavLink></li>
                            <li className='my-2 md:my-0'><NavLink to={'/contact'} className="font-semibold px-2 nav-link text-black-500 hover:text-secondary hover:underline hover:underline-offset-8">contact</NavLink></li>
                        </ul>
