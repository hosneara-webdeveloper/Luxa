...
const Orders = () => {
    const { data: orders = [], refetch } = useQuery({
        queryKey: ["orders"],
        queryFn: async () => {
            const res = await fetch('https://luxsa-back-end.vercel.app/orders');
            const data = await res.json();
            return data;
        }
    });
    const deliver = (order) => {
        fetch(`https://luxsa-back-end.vercel.app/orders/deliver/${order._id}`, {
            method: "PATCH",
            headers: {
            ...
            .then(data => {
                if (data.acknowledged || data.modifiedCount > 0) {
                    fetch(`https://luxsa-back-end.vercel.app/cart?paymentId=${order.paymentId}`, {
                        method: "DELETE"
                    })
                        .then(res => res.json())
                        .then(data => {
